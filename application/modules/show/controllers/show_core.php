...
class Show_core extends CI_controller {
    ...
    public function messages($url='')
    {
        ...
        if($url!='' && !count($_POST))
        {
            ...
            $this->db->where('url',urldecode($url));
            ...
            $data['content'] = load_view('messages_single_view', $value, TRUE);
            $data['alias'] = 'messages';
            ...
            if (count($_POST)) {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    // insert new message
                    $data = array('user_id' => (int)$user_id, 'from_id' => (int)$user_id, 'to_id' => 1, 'message' => $message, 'url' => $title, 'created' => date('Y-m-d H:i:s', time()));
                    $this->db->insert('messages', $data);

                    $this->db->where('user_id', (int)$user_id);
                    $this->db->where('url',$title);
                    $this->db->order_by('created', 'ASC');
                    ...
                    //redirect(site_url('show/messages'));
                }
            }
            ...
            $value['user_model'] = $this->user_model;
            $value['messages'] = $query;
            $data['content'] = load_view('messages_view', $value, TRUE);
            $data['alias'] = 'messages';
...

    {
    	...
    	foreach ($_POST as $key => $value) {
            ...
            {
                $value = str_replace("Aa","Å",$value);
            }
            else if(strpos($value,"Å") !== false)
            {
    ...
    public function sendpm()
    {
        ...
        $user_id = $this->session->userdata('user_id');
        if ($user_id == '') {
            echo json_encode(array('error' => true, 'message' => 'Not logged in'));
            return false;
        }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // insert new message
            $data = array('user_id' => $user_id, 'from_id' => $user_id, 'to_id' => 1,'message' => $message, 'url' => $title, 'created' => date('Y-m-d H:i:s',time()));
            $this->db->insert('messages', $data);
...
//        $this->db->or_where('to_id',$user_id);
        $this->db->where('url',urldecode($_GET['estate_title']));
        $this->db->where('user_id',$user_id);
        $this->db->order_by('created','DESC');
        $query = $this->db->get('messages');

//        $query = $this->db->get_where('messages',array('from_id'=>$user_id), 10, 0)->order_by('created', 'DESC')->get();
//echo $query->num_rows();
        if($query->num_rows() == 0) {
