...
class Account_core extends CI_Controller {
...

		{
			...
			{				
...

				{
                    ...
                    $r = $this->user_model->get_user_data_array_by_id($row->id);
    ...
    {
        $data['content'] 	= load_view('pending_payment','',TRUE,$this->active_theme);
        load_template($data,$this->active_theme);
    }
	...
	function register()
	{
		...
		else
		{
			...
			$userdata['status']		= 1;
            $userdata['payment_reference'] = $randomString;
			...
			if($enable_pricing=='Yes')
			{
				...
				else
				{
					...
					$this->session->set_userdata('amount',$package->price);
					$this->send_signup_notification_email($userdata,$unique_id);
                    $this->session->set_userdata('new_user_ref_id',$user_id);
					redirect(site_url('account/processing_payment'));
				}
			}
			else
			{
    ...
    {
        $data['content'] 	= load_view('processing_payment_view','',TRUE,$this->active_theme);
        load_template($data,$this->active_theme);
    }
    ...
    {
        $data['content'] 	= load_view('payment_view','',TRUE,$this->active_theme);
        load_template($data,$this->active_theme);
    }
    ...
    public function payment_processed()
    {
        ...
        if(isset($_REQUEST['ref'])) {
            ...
            if ($_REQUEST['ref'] == $reference) {
                ...
                $this->user_model->update_user_by_id($data,$r['id']);

                $this->session->set_userdata('user_id',$r['id']);

                $this->session->set_userdata('user_name',$r['user_name']);

                $this->session->set_userdata('user_type',$r['user_type']);

                $this->session->set_userdata('user_email',$r['user_email']);
        ...
        $data['content'] 	= load_view('payment_processed_view','',TRUE,$this->active_theme);
        load_template($data,$this->active_theme);
    }
